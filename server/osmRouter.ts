...
export default class OsmRouter {
  ...
  setupRoutes(): void {
    ...
    this.osmRouter.get("/postGISMultiOld", (req: Request, res: Response) => {
      ...
      if (bounds && query) {
        ...
        */
        ...
        const pointQuery = ServerUtils.buildPostGISQUeryOld(
          bounds,
          query,
          "points"
        );

        const wayQuery = ServerUtils.buildPostGISQUeryOld(
          bounds,
          query,
          "ways"
        );

        const polyQuery = ServerUtils.buildPostGISQUeryOld(
          bounds,
          query,
          "polygons"
        );

        const relationsQuery = ServerUtils.buildPostGISQUeryOld(
          bounds,
          query,
          "relations"
        );
        ...
        Promise.allSettled([
          ...
          pool
            ...
            .then(
              (res) =>
                (relationFeatures = res.rows[0].json_build_object.features)
            )
            .catch((e) => console.error(e)),
        ]).then((results) => {
    ...
    this.osmRouter.get("/postGISSingleOld", (req: Request, res: Response) => {
      startPerformanceMeasure("everything before", true);
      const bounds = req.query.bounds?.toString();
      ...
      if (bounds) {
        ...
        */
        ...
        const pointQuery = ServerUtils.buildPostGISQuerySingleOld(
          bounds,
          conditions,
          "points"
        );

        const wayQuery = ServerUtils.buildPostGISQuerySingleOld(
          bounds,
          conditions,
          "ways"
        );

        const polyQuery = ServerUtils.buildPostGISQuerySingleOld(
          bounds,
          conditions,
          "polygons"
        );

        const relationsQuery = ServerUtils.buildPostGISQuerySingleOld(
          bounds,
          conditions,
          "relations"
        );
    ...
    this.osmRouter.get("/postGISMulti", (req: Request, res: Response) => {
      const bounds = req.query.bounds?.toString();
      const query = req.query.osmQuery?.toString();
      const first = req.query.first?.toString();
      const last = req.query.last?.toString();
      if (first) {
        console.log(first);
        startPerformanceMeasure("RequestServer" + DBType.POSTGISMULTI, true);
      }
      //console.log("start req postgis");
      if (bounds && query) {
        /** 
        ...
        */
        ...
        Promise.allSettled([
          ...
          if (last) {
            console.log("last");
            endPerformanceMeasure("RequestServer" + DBType.POSTGISMULTI, true);
          }
          res.status(StatusCodes.OK).send(featureCollection);
        });
        // let roadFeatures: GeoJsonProperties[];
    ...
    this.osmRouter.get("/postGISIndexSp", (req: Request, res: Response) => {
      const bounds = req.query.bounds?.toString();
      const query = req.query.osmQuery?.toString();
      const first = req.query.first?.toString();
      const last = req.query.last?.toString();
      if (first) {
        startPerformanceMeasure("RequestServer" + DBType.POSTGISINDEXSp, true);
      }
      if (bounds && query) {
        /** 
        const complexQuery =
        ...
        */
        ...
        Promise.allSettled([
          ...
          if (last) {
            endPerformanceMeasure(
              "RequestServer" + DBType.POSTGISINDEXSp,
              true
            );
          }
          res.status(StatusCodes.OK).send(featureCollection);
